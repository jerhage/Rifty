...
describe("catalog seed generator", () => {
  ...
  it("rejects a card whose set is absent from the supplied set pages", async () => {
    ...
    const page = JSON.parse(await readFile(cardPagePath, "utf8")) as {
      items: Array<{ set: { set_id: string } }>;
    };
    page.items[0]!.set.set_id = "MISSING";
    await writeFile(cardPagePath, JSON.stringify(page));

    expect(() =>
      runGenerator({ CATALOG_DATA_DIRECTORY: dataDirectory, CATALOG_SEED_OUTPUT_PATH: outputPath }),
    ).toThrow(/refers to missing set MISSING/);
