...
console.log(
  `Generated ${outputPath} with ${seed.cards.length} cards, ${seed.cardPrintings.length} printings, ${seed.keywords.length} keywords (${version.slice(0, 12)}).`,
);
if (skipped.length > 0) {
  console.warn(
...
const missingImages = seed.cardMedia
  ...
  .map((row) => ({ printingId: row.printingId, imageFile: row.imageFile }));

await writeFile(missingImagesPath, `${JSON.stringify(missingImages, null, 2)}\n`, "utf8");

if (missingImages.length > 0) {
  console.warn(
    `${missingImages.length} printings have no image on disk, listed in ${missingImagesPath}:`,
  );
