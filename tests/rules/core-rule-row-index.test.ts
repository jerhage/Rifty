...
describe("coreRuleRowIndex", () => {
  it("should place every shown rule at its own row, in the order the list holds them", () => {
    ...
    expect(rows.get("100")).toBe(0);
    expect(rows.get("101")).toBe(1);
    expect(rows.get("101.1")).toBe(2);
    expect(rows.get("101.2")).toBe(3);
  });
  ...
  it("should count rows against the shown list rather than the whole document", () => {
    ...
    expect(coreRuleRowIndex(shown).get("101.2")).toBe(2);
    expect(coreRuleRowIndex(DOCUMENT).get("101.2")).toBe(3);
  });

  it("should hold no row for a number the list does not show", () => {
    const shown = DOCUMENT.filter((coreRule) => coreRule.number !== "101.1");

    expect(coreRuleRowIndex(shown).get("101.1")).toBeUndefined();
    expect(coreRuleRowIndex(DOCUMENT).get("999")).toBeUndefined();
  });
});
