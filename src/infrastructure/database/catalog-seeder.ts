...
async function insertCatalog(db: ReturnType<typeof drizzle>): Promise<void> {
  ...
  for (const rows of batches(catalogSeed.cardSupertypes)) await db.insert(cardSupertypes).values([...rows]);
  ...
  for (const rows of batches(catalogSeed.catalogCards)) await db.insert(catalogCards).values([...rows]);
  ...
  for (const rows of batches(catalogSeed.cardDomains)) await db.insert(cardDomains).values([...rows]);
